...
def main():
    ...
    if argv[1]:
        ...
        call(
            f"./criu/crit/crit-python3 decode -i /{base_state_saved}/snapshot/fdinfo-2.img --pretty -o ./fdinfo".split()
        )
        ...
        files = filter(
            lambda x: "reg" in x.keys() and "/fd/" in x["reg"]["name"],
            file_info["entries"],
        )
        ...
        for m in mapping:
            open_fds += (
                f"exec {m[0]}<> {re.sub(r'fitm-c[0-9]+s[0-9]+', argv[2], m[1])}\n"
            )
