...
if [ ! -f "../AFLplusplus/include/config.h" ]; then

  echo "[-] Error: key files not found - wrong working directory? AFLplusplus submodule checked out?"
  exit 1
...
echo "[*] Making sure FitM-qemu is checked out"

test -d FitM-qemu || { echo "[-] Not checked out, please checkout submodules using 'git submodule update --init'." ; exit 1 ; }
echo "[+] Got FitM-qemu."
...
cp "../../AFLplusplus/include/cmplog.h" "./qemuafl/imported/" || exit 1
cp "../../AFLplusplus/include/snapshot-inl.h" "./qemuafl/imported/" || exit 1
cp "../../AFLplusplus/include/types.h" "./qemuafl/imported/" || exit 1
...
cd ..
ls -l ../fitm-qemu-trace || exit 1

echo "[+] Successfully created '../fitm-qemu-trace'."
...
if [ "$ORIG_CPU_TARGET" = "" ]; then
  ...
  echo "[*] Comparing two afl-showmap outputs..."
  echo 0 | ./afl-showmap -m none -q -o .test-instr0 -- ../fitm-qemu-trace ./test-instr || exit 1
  echo 1 | ./afl-showmap -m none -q -o .test-instr1 -- ../fitm-qemu-trace ./test-instr || exit 1
  ...
  if [ "$DR" = "0" ]; then
    ...
    echo "[-] Error: fitm-qemu-trace instrumentation doesn't seem to work!"
    exit 1
